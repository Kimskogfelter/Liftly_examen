...
function EditProfileBio({ onClose, currentUser, setCurrentUser, getUserInfo }) {
    ...
    const editProfileBio = async (e) => {
        ...
        try {
            // Skicka både bio och socialLinks till backend
            const response = await api.patch(`/users/update`, {
                profileBio,
                socialLinks
            });

            console.log("Profile update request sent successfully:", response.data);

            if (response.status === 200) {
                // Plocka ut den uppdaterade användaren från svaret
                const updatedUserFromBackend = response.data.updatedUser;

                // Skapa nytt objekt för React-state & localStorage
                const updatedUser = {
                    ...currentUser,
                ...
                // Uppdatera React state och LocalStorage
                setCurrentUser(updatedUser);
                localStorage.setItem("currentUser", JSON.stringify(updatedUser));

                // Hämta om profil-infon så vyn uppdateras direkt
                await getUserInfo();
            }
        ...
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 font-sans">
                ...
                <div className="w-full max-w-md bg-white rounded-xl p-5 shadow-xl border border-gray-100 animate-in fade-in zoom-in-95 duration-150">
                    ...
                    <h3 className="text-sm font-bold text-gray-900 mb-3 text-left">Edit Profile</h3>
                    ...
                    <form onSubmit={editProfileBio} className="space-y-4">
                        ...
                        <div className="relative">
                            ...
                            <textarea
                                ...
                                className="w-full min-h-24 p-3 text-gray-800 placeholder-gray-400 border border-gray-200 rounded-lg resize-none focus:outline-none text-xs bg-gray-50/50 transition-all focus:border-zinc-400"
                            ></textarea>
                        </div>
